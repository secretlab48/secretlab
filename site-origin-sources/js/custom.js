...
if ( $('body').hasClass( 'home' ) ) {
    ...
    let ps = gsap.utils.toArray('.os-box, .hww-item-box, .fpbg.mp-5');
    ps.forEach((p) => {
        gsap.from(p, {
            scrollTrigger: {
...
$(document).ready( function() {
    ...
    if ( $('.portfolios-items-box').length > 0 ) {
        ...
        $( 'a.portfolio-item img' ).imagesLoaded( function() {
            ...
            let ps = gsap.utils.toArray('.portfolio-item');
            ps.forEach((p) => {
                gsap.from(p, {
                    scrollTrigger: {
                        start: 'top bottom',
    ...
    if ( $('.posts-archive-items-box').length > 0 ) {
        ...
        $( '.posts-archive-items-box img' ).imagesLoaded( function() {
            ...
            if ($('.posts-archive-items-box').length > 0) {
                ...
                let ps = gsap.utils.toArray('.post-archive-item-box');
                ps.forEach((p) => {
                    gsap.from(p, {
                        scrollTrigger: {
                            start: 'top bottom',
    ...
    $( 'form.cf' ).on( 'submit', function( e )  {
        ...
        if ( Object.keys(errors).length > 0 ) {
            ...
            $.ajax({
                'method': 'POST',
                'url': ajaxdata.url,
                'data': { 'action': 'udft_cf_request', 'data' : data },
