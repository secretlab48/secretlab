...
$theme = get_sub_field( 'testimonials_with_image_slider_section_theme' );
switch ( $theme ) {
    case 'green' :
...
$query_type = get_sub_field( 'testimonials_with_image_slider_section_testimonials_query_type' );
if ( $query_type == 'last' ) {
    $ppp = get_sub_field( 'testimonials_with_image_slider_section_last_testimonials_quantity' );
    $testimonials = get_posts( array( 'post_type' => 'testimonials', 'posts_per_page' => 5, 'suppress_filter' => false, ) );
}
else if ( $query_type == 'selected' ) {
    $testimonials = get_sub_field( 'testimonials_with_image_slider_section_select_testimonials' );
}
