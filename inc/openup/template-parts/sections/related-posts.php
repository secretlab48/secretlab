<?php
global $post;
$post_name = get_queried_object()->post_type;
$related_posts_title = __('Lees ook deze artikelen', 'openup');
$tax_query = [];
if ($post_name == 'business_post'){
    $types_terms = wp_get_post_terms( $post->ID, 'business_posts_types_tax', [ 'fields' => 'ids' ] );
    $taxonomy = 'business_posts_types_tax';
    $post_type = 'business_post';
}elseif ($post_name == 'case_study'){
    $related_posts_title = __('Also read these other case studies', 'openup');
    $types_terms = wp_get_post_terms( $post->ID, 'case_studies_tax', [ 'fields' => 'ids' ] );
